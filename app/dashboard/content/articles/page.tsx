...
export default function ArticlesPage() {
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      ...
      toast('Article deleted successfully', { className: 'bg-green-500' });
    } catch (err: any) {
      toast(err.message || 'Failed to delete article', { className: 'bg-red-500' });
      console.error(err);
    }
  };
  ...
  if (!user) {
    return (
      <div className="bg-yellow-500/10 border border-yellow-500/50 p-6 rounded-lg text-center">
        ...
        <p className="text-gray-300">Please sign in to view and manage your articles.</p>
      </div>
    );
  }
